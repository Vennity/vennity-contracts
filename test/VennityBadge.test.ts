...
describe(`VennityBadge`, () => {
  ...
  describe(`VennityBadge 0th Edition`, async () => {
    ...
    it(`should give the initial supply to the creator's address`, async () => {
      const balance = await VennityBadge.balanceOf(deployerAddress)
    })
    ...
    describe(`safeTransferFrom(...)`, () => {
      ...
      it(`should succeed when the sender has enough balance`, async () => {
        ...
        const deployerBalance: BigNumber = await VennityBadge.balanceOf(deployerAddress)
        const recipientBalance: BigNumber = await VennityBadge.balanceOf(recipientAddress)
